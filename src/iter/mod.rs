...
///
/// Note that the overhead of `ParallelIterator` is high relative to some workloads. In particular,
/// if the batch size is too small or task being run in parallel is inexpensive, a
/// `ParallelIterator` could take longer than a normal `std::iter::Iterator`. Therefore, you should
/// profile your code before using `ParallelIterator`.
    ...
    Self: Sized + Send,
{
    ...
    ///
    /// Each batch is an iterator with items of the same type as the `ParallelIterator`.
    /// Returns `None` when there are no batches left.
    fn next_batch(&mut self) -> Option<B>;

    /// Returns the bounds on the remaining number of items in the `ParallelIterator`.
    ///
    ...
    /// Consumes the `ParallelIterator` and returns the number of items.
    ///
    ...
    /// Consumes the `ParallelIterator` and returns the last item.
    ///
    ...
    /// Consumes the `ParallelIterator` and returns the nth item.
    ///
    ...
    /// Takes two `ParallelIterator`s and returns a `ParallelIterator` over both in sequence.
    ///
    ...
    /// Takes a closure and creates a `ParallelIterator` which calls that closure on each item.
    ///
    ...
    /// Calls a closure on each item of a `ParallelIterator`.
    ///
    ...
    /// Creates a `ParallelIterator` which uses a closure to determine if an element
    /// should be yielded.
    ///
    ...
    /// Creates a `ParallelIterator` that both filters and maps.
    ///
    ...
    /// Creates a `ParallelIterator` that works like map, but flattens nested structure.
    ///
    ...
    /// Creates a `ParallelIterator` that flattens nested structure.
    ///
    ...
    /// Creates a `ParallelIterator` which ends after the first None.
    ///
    ...
    /// Does something with each item of a `ParallelIterator`, passing the value on.
    ///
    ...
    /// Borrows a `ParallelIterator`, rather than consuming it.
    ///
    ...
    /// Transforms a `ParallelIterator` into a collection.
    ///
    ...
    /// Consumes a `ParallelIterator`, creating two collections from it.
    ///
    ...
    /// Repeatedly applies a function to items of each batch of a `ParallelIterator`, producing
    /// a Vec of final values.
    ///
    /// Folds each batch independently and returns a Vec of results (in batch order).
    ///
    ...
    /// Tests if every element of the `ParallelIterator` matches a predicate.
    ///
    /// `all` is **not** short circuiting.
    ///
    ...
    /// Tests if any element of the `ParallelIterator` matches a predicate.
    ///
    /// `any` is **not** short circuiting.
    ///
    ...
    /// Searches for an element in a `ParallelIterator`, returning its index.
    ///
    ...
    /// Returns the maximum item of a `ParallelIterator`.
    ///
    ...
    /// Returns the minimum item of a `ParallelIterator`.
    ///
    ...
    /// Creates a `ParallelIterator` which copies all of its items.
    ///
    ...
    /// Creates a `ParallelIterator` which clones all of its items.
    ///
    ...
    /// Sums the items of a `ParallelIterator`.
    ///
    ...
    /// Multiplies all the items of a `ParallelIterator`.
    ///
